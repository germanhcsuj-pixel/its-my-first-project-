...
// Обработка возврата после Google redirect (Р В РЎВР В РЎвЂўР В Р’В±Р В РЎвЂР В В»Р РЋР Р‰Р В Р вЂ¦Р РЋРІР‚в„–Р В Р’Вµ)
if (typeof firebase !== 'undefined' && firebase.auth) {
...
let authMode = ' или '; // 'login' Р В РЎвЂР В В»Р В РЎвЂ 'register'
...
window.submitAuth = function() {
    ...
    action
        ...
        .catch((err) => {
            const msgs = {
                'Пользователь не найден': 'Р В РЎСџР В РЎвЂўР В В»Р РЋР Р‰Р В Р’В·Р В РЎвЂўР В Р вЂ Р В Р’В°Р РЋРІР‚С™Р В Р’ВµР В В»Р РЋР Р‰ Р В Р вЂ¦Р В Р’Вµ Р В Р вЂ¦Р В Р’В°Р В РІвЂћвЂ“Р В РўвЂР В Р’ВµР В Р вЂ¦',
                'Неверный пароль': 'Р В РЎСљР В Р’ВµР В Р вЂ Р В Р’ВµР РЋР вЂљР В Р вЂ¦Р РЋРІР‚в„–Р В РІвЂћвЂ“ Р В РЎвЂ”Р В Р’В°Р РЋР вЂљР В РЎвЂўР В В»Р РЋР Р‰',
                'Email уже используется': 'Email Р РЋРЎвЂњР В Р’В¶Р В Р’Вµ Р В РЎвЂР РЋР С“Р В РЎвЂ”Р В РЎвЂўР В В»Р РЋР Р‰Р В Р’В·Р РЋРЎвЂњР В Р’ВµР РЋРІР‚С™Р РЋР С“Р РЋР РЏ',
                'Неверный формат email': 'Р В РЎСљР В Р’ВµР В Р вЂ Р В Р’ВµР РЋР вЂљР В Р вЂ¦Р РЋРІР‚в„–Р В РІвЂћвЂ“ Р РЋРІР‚С›Р В РЎвЂўР РЋР вЂљР В РЎВР В Р’В°Р РЋРІР‚С™ email',
                'Пароль слишком слабый': 'Р В РЎСџР В Р’В°Р РЋР вЂљР В РЎвЂўР В В»Р РЋР Р‰ Р РЋР С“Р В В»Р В РЎвЂР РЋРІвЂљВ¬Р В РЎвЂќР В РЎвЂўР В РЎВ Р РЋР С“Р В В»Р В Р’В°Р В Р’В±Р РЋРІР‚в„–Р В РІвЂћвЂ“',
                'РќРµРІРµСЂРЅС‹Р№ email или РїР°СЂРѕР»СЊ': 'Р В РЎСљР В Р’ВµР В Р вЂ Р В Р’ВµР РЋР вЂљР В Р вЂ¦Р РЋРІР‚в„–Р В РІвЂћвЂ“ email Р В РЎвЂР В В»Р В РЎвЂ Р В РЎвЂ”Р В Р’В°Р РЋР вЂљР В РЎвЂўР В В»Р РЋР Р‰',
            };
...
function renderQuickPills() {
  ...
  const selected = [
    ...
    { icon: 'Что такое машинное обучение?', text: "Р В Р’В§Р РЋРІР‚С™Р В РЎвЂў Р РЋРІР‚С™Р В Р’В°Р В РЎвЂќР В РЎвЂўР В Р’Вµ Р В РЎВР В Р’В°Р РЋРІвЂљВ¬Р В РЎвЂР В Р вЂ¦Р В Р вЂ¦Р В РЎвЂўР В Р’Вµ Р В РЎвЂўР В Р’В±Р РЋРЎвЂњР РЋРІР‚РЋР В Р’ВµР В Р вЂ¦Р В РЎвЂР В Р’Вµ?" }
  ];
  container.innerHTML = '';
...
window.openAttachmentPreview = function(src) {
    if (!document.getElementById('attachmentLightboxCss')) {
        const style = document.createElement('style');
        style.id = 'attachmentLightboxCss';
        style.innerHTML = `
            #attachmentLightbox {
                position: fixed;
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            ...
            <button type="button" class="attachment-lightbox-download" style="position:absolute; bottom:30px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,0.6); color:#fff; border:1px solid rgba(255,255,255,0.2); padding:10px 20px; border-radius:20px; cursor:pointer; font-size:14px; display:flex; align-items:center; gap:8px; z-index:100000; backdrop-filter:blur(10px);">
        ...
        modal.addEventListener('click', (event) => {
            ...
            } else if (event.target.closest('.attachment-lightbox-download')) {
                const imgSrc = modal.querySelector('.attachment-lightbox-image').src;
                fetch(imgSrc).then(res => res.blob()).then(blob => {
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.style.display = 'none';
                    a.href = url;
                    a.download = 'solifon_image_' + Date.now() + '.jpg';
...
document.addEventListener('click', (event) => {
    const img = event.target.closest('#imagePreviewContainer img, .message img');
...
function mhOpenAI(role) {
  ...
  if (role === 'parent') {
    ...
    greeting = `Р—РґСЂР°РІСЃС‚РІСѓР№С‚Рµ! РЇ SoulDrive.\n\nР—Р°РїРёСЃСЊ РїРѕ СЂРµР±С‘РЅРєСѓ **${mhCurrentChild.fio || 'РЅРµ СѓРєР°Р·Р°РЅ'}**. Р§РµРј РјРѕРіСѓ РїРѕРјРѕС‡СЊ? РњРѕРіСѓ РїСЂРµРґР»РѕР¶РёС‚СЊ РґРѕРјР°С€РЅРёРµ СѓРїСЂР°Р¶РЅРµРЅРёСЏ, РѕС‚РІРµС‚РёС‚СЊ РЅР° РІРѕРїСЂРѕСЃС‹ Рѕ СЂР°Р·РІРёС‚РёРё или РїРѕРґРґРµСЂР¶Р°С‚СЊ РІР°СЃ.`;
...
(function () {
  ...
  const dict = {
    ru: {
      ...
      questions: [
        ['Что такое искусственный интеллект?', 'Р В Р’В§Р РЋРІР‚С™Р В РЎвЂў Р РЋРІР‚С™Р В Р’В°Р В РЎвЂќР В РЎвЂўР В Р’Вµ Р В РЎвЂР РЋР С“Р В РЎвЂќР РЋРЎвЂњР РЋР С“Р РЋР С“Р РЋРІР‚С™Р В Р вЂ Р В Р’ВµР В Р вЂ¦Р В Р вЂ¦Р РЋРІР‚в„–Р В РІвЂћвЂ“ Р В РЎвЂР В Р вЂ¦Р РЋРІР‚С™Р В Р’ВµР В В»Р В В»Р В Р’ВµР В РЎвЂќР РЋРІР‚С™?'],
        ...
        ['Что такое машинное обучение?', 'Р В Р’В§Р РЋРІР‚С™Р В РЎвЂў Р РЋРІР‚С™Р В Р’В°Р В РЎвЂќР В РЎвЂўР В Р’Вµ Р В РЎВР В Р’В°Р РЋРІвЂљВ¬Р В РЎвЂР В Р вЂ¦Р В Р вЂ¦Р В РЎвЂўР В Р’Вµ Р В РЎвЂўР В Р’В±Р РЋРЎвЂњР РЋРІР‚РЋР В Р’ВµР В Р вЂ¦Р В РЎвЂР В Р’Вµ?']
      ]
    },
    kk: {
      download: 'Solifon AI Р¶РўР‡РєС‚РµСѓ',
...
(function() {
    ...
    const generatedData = {
        "ru": [
            ...
            "Қалайсың?",
            "Маған көмектесші",
            "Напиши подробный бизнес-план для открытия кофейни с финансовыми расчетами",
            "Жасанды интеллекттің болашағы туралы аналитикалық эссе жазып бер",
            "Как настроить CI/CD pipeline для React приложения через GitHub Actions?",
            "Қазақстандағы IT саласының дамуы мен келешегі туралы ғылыми мақала",
            "Объясни сложные концепции квантовых вычислений простыми словами для школьника",
            "Түйіндемемді (CV) халықаралық IT компаниялардың талабына сай етіп өңдеп бер",
            "Сгенерируй полноценный код игры 'Змейка' на Python с использованием Pygame",
            "Python тілінде көп функциялы Telegram бот жазудың толық нұсқаулығын құрастыр",
            "Составь персонализированный план тренировок и питания на месяц для набора мышечной массы",
            "Қазақ тіліндегі грамматиканы автоматты түрде тексеретін алгоритм қалай жұмыс істейді?",
            "Помоги подготовиться к сложному техническому собеседованию на позицию Junior Frontend Developer",
            "Жаңа технологиялық стартапқа қандай заманауи маркетингтік стратегиялар қолданған дұрыс?",
            "Напиши масштабируемый скрипт на Node.js для асинхронного парсинга данных с веб-страниц",
            "Көркем әдебиет стилінде ғарышқа саяхат туралы кішігірім фантастикалық әңгіме жазып бер",
            "Как именно работает алгоритм рекомендаций контента в таких сетях как TikTok и YouTube?",
            "Маған шетелдік университетке магистратураға түсу үшін өте мықты мотивациялық хат жазуға көмектесші",
            "Переведи этот сложный академический текст по нейросетям на понятный русский язык без потери смысла",
            "React, Vue.js және Angular фреймворктерінің архитектуралық артықшылықтары мен кемшіліктерін салыстыр",
            "Создай нормализованную структуру реляционной базы данных PostgreSQL для крупного интернет-магазина",
            "Қазақ ұлттық ертегілерінің желісімен қазіргі заманға сай толықметражды фильмнің сценарийін құрастырып бер"
        ],
        "en": [
...
window.selectDeepCmd = function(cmd) {
    ...
    if (cmd === 'docx') {
        window._deepMode = 'docx_create';
        ta.value = '/word create ';
        ta.placeholder = 'Опишите содержимое документа Word...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        _showDeepTag('\uD83D\uDCC4 /word create', '#4a90e2');
    } else if (cmd === 'xlsx') {
        window._deepMode = 'xlsx_create';
        ta.value = '/excel create ';
        ta.placeholder = 'Опишите содержимое таблицы Excel...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        _showDeepTag('\uD83D\uDCCA /excel create', '#48c78e');
    } else if (cmd === 'pptx') {
        window._deepMode = 'pptx_create';
        ta.value = '/ppt create ';
        ta.placeholder = 'Опишите презентацию PowerPoint...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        _showDeepTag('\uD83D\uDCFC /ppt create', '#fc814a');
    } else if (cmd === 'pdf_create') {
        window._deepMode = 'pdf_create';
        window._pdfFileForEdit = null;
        ta.value = '/pdf create ';
        ta.placeholder = 'Опишите содержимое PDF... (отчет, статья, расписание...)';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        ...
        window._pdfFileForEdit = null;
        _showDeepTag('\u270F\uFE0F /pdf edit — Выберите PDF', '#9a75f3');
        ta.value = '/pdf edit ';
        ta.placeholder = 'Что нужно изменить в PDF...';
        ta.focus();
        ...
        window._deepMode = null;
        ta.value = '/video ';
        ta.placeholder = 'Вставьте ссылку на YouTube видео...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        ...
        window._deepMode = null;
        ta.value = '/image ';
        ta.placeholder = 'Опишите изображение для генерации...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
        ...
        window._deepMode = null;
        ta.value = '/web ';
        ta.placeholder = 'Поиск в интернете...';
        ta.focus();
        ta.dispatchEvent(new Event('input'));
...
window.handlePdfFileSelect = function(input) {
    ...
    _showDeepTag('\u270F\uFE0F /pdf edit — ' + file.name, '#9a75f3');
    const ta = document.getElementById('userInput');
    if (ta) {
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    window.handleAI = async function() {
        ...
        if (mode === 'docx_create' || text.toLowerCase().startsWith('/word create')) {
            const prompt = text.replace(/^\/word create\s*/i, '').trim();
            if (!prompt) { if (ta) ta.placeholder = 'Опишите содержимое Word...'; return; }
            await _callDocxCreate(prompt);
            window._clearDeepMode();
            return;
        }
        if (mode === 'xlsx_create' || text.toLowerCase().startsWith('/excel create')) {
            const prompt = text.replace(/^\/excel create\s*/i, '').trim();
            if (!prompt) { if (ta) ta.placeholder = 'Опишите содержимое Excel...'; return; }
            await _callXlsxCreate(prompt);
            window._clearDeepMode();
            return;
        }
        if (mode === 'pptx_create' || text.toLowerCase().startsWith('/ppt create')) {
            const prompt = text.replace(/^\/ppt create\s*/i, '').trim();
            if (!prompt) { if (ta) ta.placeholder = 'Опишите презентацию...'; return; }
            await _callPptxCreate(prompt);
            window._clearDeepMode();
            return;
        }
    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', function(e) {
            ...
            if (mode === 'pdf_create' || mode === 'pdf_edit' ||
                mode === 'docx_create' || mode === 'xlsx_create' || mode === 'pptx_create' ||
                text.toLowerCase().startsWith('/pdf create') ||
                text.toLowerCase().startsWith('/pdf edit') ||
                text.toLowerCase().startsWith('/word create') ||
                text.toLowerCase().startsWith('/excel create') ||
                text.toLowerCase().startsWith('/ppt create')) {
...
async function _callPdfCreate(prompt, filesToAttach = []) {
    ...
    try {
        ...
        try {
            ...
            chatFd.append('prompt', 'Напиши короткое элегантное вступление (2-3 предложения) о том, что ты создал документ PDF на тему: ' + prompt + '. Не используй заголовки.');
        ...
        if (aiText) {
            _updateBotMsg(botEl, aiText + '<br><br>⏳ <i>Генерирую документ...</i>');
        }
        ...
        if (data.raw) {
            ...
            const escapedMarkdown = encodeURIComponent(rawMarkdown);
            
            // Generate the document card HTML
            const persistentCardHtml = `
                <div class="doc-card" onclick="window.openDocumentEditor('${escapedMarkdown}', '${pdfName}')">
                    <div class="doc-card-left">
                        <div class="doc-card-icon"><i class="ph ph-file-pdf"></i></div>
                        <div class="doc-card-info">
                            <span class="doc-card-title">Document Ready</span>
                            <span class="doc-card-subtitle">${pdfName}</span>
                        </div>
                    </div>
                </div>
            `;
            
            _updateBotMsg(botEl, (aiText ? aiText + persistentCardHtml : persistentCardHtml));
            if (typeof saveToFirebase === 'function') {
                saveToFirebase('ai', (aiText ? aiText + persistentCardHtml : persistentCardHtml));
            }
        } else {
            const finalHtml = (aiText ? aiText + '<br><br>' : '') + (data.reply || data.error || 'Готово!');
            _updateBotMsg(botEl, finalHtml);
            if (typeof saveToFirebase === 'function') saveToFirebase('ai', finalHtml);
        }
    } catch (err) {
        const errorMsg = '❌ Ошибка создания PDF: ' + err.message;
...
// Close panel when clicking in chat area (outside the panel)
setTimeout(() => {
    const mainChat = document.querySelector('.main-content') || document.getElementById('chatWrapper');
    if (mainChat) {
