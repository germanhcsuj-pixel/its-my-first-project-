...
export class ScenesManager {
	...
	async addMarker(markerData: Omit<Marker, "id">): Promise<string> {
		if (!this.editor.project.getActive()) {
			throw new Error("No active project");
		}
		const command = new AddMarkerCommand(markerData);
		this.editor.command.execute({ command });
		return command.getMarkerId();
	}
	...
	async removeMarker(markerId: string): Promise<void> {
		...
		const command = new RemoveMarkerCommand(markerId);
		this.editor.command.execute({ command });
	}
