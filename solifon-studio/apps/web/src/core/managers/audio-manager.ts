...
export class AudioManager {
	...
	private async scheduleAllClips({
		...
		// Calculate ducking envelopes based on TextTracks (subtitles)
		const textTracks = this.editor.timeline.getTracks().filter(t => t.type === "text" && !t.hidden);
		const duckingEnvelopes = textTracks.flatMap(t => 
			t.elements.map(e => ({ start: e.startTime, end: e.startTime + e.duration }))
		);
		...
		for (const clip of this.clips) {
			...
			try {
				...
				this.scheduleClipNode({ clip, buffer, time, duckingEnvelopes });
			} catch (error) {
	...
	private scheduleClipNode({
		...
		buffer: AudioBuffer;
		time: number;
		duckingEnvelopes?: { start: number; end: number }[];
	}): void {
		const audioContext = this.audioContext;
		...
		// Apply Audio Ducking for background tracks
		if (clip.isBackground && duckingEnvelopes.length > 0) {
			const DUCKING_VOLUME = clip.volume * 0.2;
			const RAMP_TIME = 0.3; // 300ms fade
			
			// Initial state (we just set it for the whole timeline, it will only affect what plays)
			clipGain.gain.setValueAtTime(clip.volume, Math.max(0, scheduleTime - 1));
			
			for (const envelope of duckingEnvelopes) {
				const envStartContextTime = this.playbackStartContextTime + (envelope.start - this.playbackStartTime);
				const envEndContextTime = this.playbackStartContextTime + (envelope.end - this.playbackStartTime);
				
				// Only schedule if the envelope is in the future relative to our current playback
				if (envEndContextTime > audioContext.currentTime) {
					// Fade out when speech starts
					clipGain.gain.setValueAtTime(clip.volume, Math.max(audioContext.currentTime, envStartContextTime - RAMP_TIME));
					clipGain.gain.linearRampToValueAtTime(DUCKING_VOLUME, Math.max(audioContext.currentTime, envStartContextTime));
					
					// Fade back in when speech ends
					clipGain.gain.setValueAtTime(DUCKING_VOLUME, Math.max(audioContext.currentTime, envEndContextTime));
					clipGain.gain.linearRampToValueAtTime(clip.volume, Math.max(audioContext.currentTime, envEndContextTime + RAMP_TIME));
				}
			}
		}
