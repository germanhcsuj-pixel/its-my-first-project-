...
export class TimelineManager {
	...
	addTransition({
		...
		// Проверка с мягким допуском — принимаем клипы с зазором до 0.5с или перекрытием
		if (!areElementsAdjacent({ elementA: fromElement as any, elementB: toElement as any })) {
			return null;
		}
