...
function applyTransitionToAdjacentPairs({
	...
	console.log("[Transition Debug] selectedElements:", selectedElements);
	console.log("[Transition Debug] all tracks:", tracks.map(t => ({ id: t.id, type: t.type, elemCount: t.elements.length })));

	// If user has a specific clip selected, apply transition to it
	if (selectedElements.length > 0) {
		for (const selected of selectedElements) {
			const track = tracks.find((t) => t.id === selected.trackId);
			console.log("[Transition Debug] selected:", selected, "track found:", !!track);
			if (!track || track.type === "audio") continue;

			const pairs = findAdjacentPairs({ track });
			console.log("[Transition Debug] adjacent pairs in track:", pairs.map(p => ({ from: p.from.id, to: p.to.id })));
			
			const relevantPairs = pairs.filter(p => p.from.id === selected.elementId || p.to.id === selected.elementId);
			console.log("[Transition Debug] relevant pairs:", relevantPairs.length);
			...
			if (relevantPairs.length > 0) {
				for (const pair of relevantPairs) {
					...
					console.log("[Transition Debug] addTransition result:", result);
					if (result) applied++;
				}
			} else {
				// No adjacent clips, apply transition directly to this clip
	...
	// Fallback to applying to ALL pairs if nothing is selected
