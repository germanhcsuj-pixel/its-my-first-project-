...
import { useAIAudioGenerationStore } from "@/stores/ai-audio-generation-store";
...
function AIAudioView() {
	const { t } = useTranslation();
	const { prompt, modelId, isGenerating, generatedAudios, setPrompt, setModelId, generate, removeAudio } = useAIAudioGenerationStore();
	const editor = useEditor();

	const handleAddToTimeline = async (audio: any) => {
		if (!audio.audioUrl) return;
		try {
			const res = await fetch(audio.audioUrl);
			const blob = await res.blob();
			const file = new File([blob], `generated-audio-${audio.id}.wav`, { type: "audio/wav" });
			...
			if (activeProject) {
				...
				toast.success(t("Added to media assets!"));
			}
		} catch(e) {
			toast.error(t("Failed to add audio to assets."));
		}
	};
		...
		<div className="flex flex-col gap-4 py-4 px-1">
			<div className="flex flex-col gap-2">
				<p className="text-sm text-foreground">
					{t("Describe the audio or sound effect you want to generate.")}
				</p>
				<Textarea
					placeholder={t("e.g. A loud laser blast")}
					value={prompt}
				...
				<label className="text-xs text-muted-foreground mt-2">{t("Model")}</label>
				<Select value={modelId} onValueChange={setModelId} disabled={isGenerating}>
					<SelectTrigger>
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
						<SelectItem value="Xenova/musicgen-small">{t("Small (Fast, Lower Quality)")}</SelectItem>
						<SelectItem value="Xenova/audioldm-s-full-v2">{t("Large (Slower, Higher Quality)")}</SelectItem>
					</SelectContent>
				</Select>
			...
			{generatedAudios.length > 0 && (
				<div className="flex flex-col gap-2 mt-4">
					<span className="text-xs text-muted-foreground font-medium">
						{t("Generated Audios ({{num}})", { num: generatedAudios.length })}
					</span>
					<div className="flex flex-col gap-2">
						{generatedAudios.map((audio) => (
							<div key={audio.id} className="bg-muted/50 rounded-md border p-3 flex flex-col gap-2 relative group">
								<button
									type="button"
								...
								<p className="text-xs font-medium pr-6">{audio.prompt}</p>
								
								{audio.status === "complete" && audio.audioUrl && (
									<>
										{/* biome-ignore lint/a11y/useMediaCaption: audio only */}
										<audio src={audio.audioUrl} controls className="w-full h-8 mt-1" />
										<Button size="sm" variant="outline" className="w-full mt-1" onClick={() => handleAddToTimeline(audio)}>
											{t("Add to Media Assets")}
										</Button>
									</>
								)}
								{(audio.status === "running" || audio.status === "pending") && (
									<div className="flex items-center gap-2 mt-1">
										<HugeiconsIcon icon={Loading03Icon} className="size-4 animate-spin text-blue-500" />
										<span className="text-xs text-muted-foreground">
											{audio.progress ? `${Math.round(audio.progress)}%` : t("Generating...")}
										</span>
									</div>
								)}
								{audio.status === "failed" && (
									<span className="text-xs text-red-500 mt-1">{audio.error || t("Failed")}</span>
								)}
							</div>
						))}
					</div>
...
export function AutoCaptionsView() {
	const { t } = useTranslation();
	const editor = useEditor();
	...
	const generateCaptions = async () => {
		...
		if (selectedElements.length === 0) {
			setStatusMsg(t("Please select a video or audio element first."));
			return;
		}

		const elementObj = selectedElements[0];
		const tracks = editor.timeline.getTracks();
		let targetElement = null;
		
		for (const track of tracks) {
			const found = track.elements.find((e: any) => e.id === elementObj.elementId);
			if (found) {
				targetElement = found;
				break;
			}
		...
		if (!targetElement || (targetElement.type !== "video" && targetElement.type !== "audio")) {
			setStatusMsg(t("Selected element must be video or audio."));
			return;
		}

		const mediaId = (targetElement as any).mediaId;
		if (!mediaId) {
			setStatusMsg(t("No media associated with element."));
			return;
		}
		...
		if (!media || !media.url) {
			setStatusMsg(t("Media file not found."));
			return;
		}

		try {
			setIsProcessing(true);
			setStatusMsg(t("Downloading media..."));

			const response = await fetch(media.url);
			const arrayBuffer = await response.arrayBuffer();

			setStatusMsg(t("Extracting audio to 16kHz mono..."));
			const { WhisperClient } = await import("@/lib/ai/whisper-client");
			const audioData = await WhisperClient.extractAudioTo16kHzMono(arrayBuffer);

			setStatusMsg(t("Loading Whisper model..."));
			const whisper = new WhisperClient();
			await whisper.load(model, (progress) => {
				if (progress.status === "progress") {
					setStatusMsg(t(`Downloading model... ${Math.round(progress.progress)}%`));
				}
			});

			setStatusMsg(t("Transcribing audio (this may take a while)..."));
			const result = await whisper.transcribe(audioData, model, language);

			if (result.chunks && result.chunks.length > 0) {
				setStatusMsg(t("Adding subtitles to timeline..."));
				const { buildTextElement } = await import("@/lib/timeline/element-utils");
				
				const currentTime = targetElement.startTime;
				for (const chunk of result.chunks) {
					const [startOffset, endOffset] = chunk.timestamp;
					const chunkStartTime = currentTime + startOffset;
				...
				setStatusMsg(t("Captions generated successfully!"));
			} else {
				setStatusMsg(t("No speech detected."));
			}
		} catch (error) {
			console.error(error);
			setStatusMsg(t(`Error: ${String(error)}`));
		} finally {
	...
	return (
		<div className="flex flex-col gap-4 py-4">
			<div className="flex flex-col gap-4">
				<p className="text-sm text-foreground">
					{t("Select a video or audio track on the timeline, then click Generate to create auto-captions.")}
				</p>

				<div className="flex flex-col gap-2">
					<label className="text-xs text-muted-foreground">{t("Model Size")}</label>
					<Select value={model} onValueChange={setModel} disabled={isProcessing}>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
							<SelectItem value="Xenova/whisper-tiny">{t("Tiny (Fastest, less accurate)")}</SelectItem>
							<SelectItem value="Xenova/whisper-base">{t("Base (Balanced)")}</SelectItem>
							<SelectItem value="Xenova/whisper-small">{t("Small (More accurate, slower)")}</SelectItem>
						</SelectContent>
					</Select>
				</div>
				...
				<div className="flex flex-col gap-2">
					...
					<Select value={language} onValueChange={setLanguage} disabled={isProcessing}>
						...
						<SelectContent>
							...
							<SelectItem value="kk">{t("Kazakh")}</SelectItem>
...
export function RemoveBackgroundView() {
	const { t } = useTranslation();
	const editor = useEditor();
	const [mode, setMode] = useState("auto"); // easy/hard/auto
	...
	const handleRemoveBackground = () => {
		...
		editor.timeline.updateElements({
			updates: selected.map((s) => ({
				...
				updates: { removeBackground: true },
			})),
		});

		toast.success(t("Background removal enabled for selected element."));
	};
	...
	return (
		<div className="flex flex-col gap-4 p-4">
			<div className="flex flex-col gap-2">
				<p className="text-sm text-foreground">
					{t("Select an image or video on the timeline to remove its background using AI.")}
				</p>
				<label className="text-xs text-muted-foreground mt-2">{t("Processing Mode")}</label>
				<Select value={mode} onValueChange={setMode}>
					<SelectTrigger>
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
						<SelectItem value="auto">{t("Auto (Balanced)")}</SelectItem>
						<SelectItem value="fast">{t("Fast (Lower Quality)")}</SelectItem>
						<SelectItem value="accurate">{t("Accurate (Slower)")}</SelectItem>
					</SelectContent>
				</Select>
			</div>
			...
			>
				...
				<div className="flex flex-col items-start">
					<span className="font-medium">{t("Remove Background")}</span>
					<span className="text-xs text-muted-foreground">{t("Apply AI background removal")}</span>
				</div>
			</Button>
		</div>
	);
...
import { AIEditPreviewView } from "./ai-edit-preview";
...
export function AIView() {
	...
	return (
		<BaseView
			defaultTab="smart-edit"
			tabs={[
				{
					value: "smart-edit",
					label: t("Smart Edit"),
					content: <AIEditPreviewView />,
				},
				{
					value: "generate",
					label: t("Generate"),
				...
				},
				{
					value: "remove-bg",
					label: t("Remove BG"),
					content: <RemoveBackgroundView />,
				},
				{
					value: "ai-captions",
					label: t("Auto Captions"),
					content: <AutoCaptionsView />,
				},
				{
					value: "ai-audio",
					label: t("Audio AI"),
