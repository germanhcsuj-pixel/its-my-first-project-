			...
			<Tabs defaultValue="style" className="flex h-full flex-col">
				...
				<TabsContent value="style" className="mt-0 flex-1 overflow-auto">
					<PanelBaseView className="p-0">
						<InlineAnimationPicker element={element} trackId={elementRefs[0].trackId} />
						<PropertyGroup
							title={t("Content")}
							hasBorderTop={false}
							collapsible={false}
						...
						<PropertyGroup title={t("Appearance")} collapsible={false}>
							<div className="space-y-6">
								...
								<PropertyItem direction="column">
									<PropertyItemLabel>{t("Box Width (Wrap)")}</PropertyItemLabel>
									<PropertyItemValue>
										<div className="flex items-center gap-2">
											<Slider
												value={[element.boxWidth ?? 0]}
												min={0}
												max={1920}
												step={10}
												onValueChange={([value]) => {
													editor.timeline.updateElements({
														updates: buildBatchUpdates({
															boxWidth: value === 0 ? undefined : value,
														}),
														pushHistory: false,
													});
												}}
											...
											<span className="text-muted-foreground w-12 text-center text-xs">
												{element.boxWidth ? `${element.boxWidth}px` : "Auto"}
											</span>
										</div>
									</PropertyItemValue>
								</PropertyItem>
