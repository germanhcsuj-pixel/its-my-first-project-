...
export function StickerProperties({
	...
	const opacityDisplay = isEditingOpacity.current
		...
		: Math.round(((element as any).opacity ?? 1) * 100).toString();
									...
									<Slider
										...
										onValueChange={([value]) => {
											if (initialOpacityRef.current === null) {
												initialOpacityRef.current = (element as any).opacity ?? 1;
											}
											updateElement({
										...
										onBlur={() => {
											if (initialOpacityRef.current !== null) {
												...
												const opacityPercent = Number.isNaN(parsed)
													? Math.round(((element as any).opacity ?? 1) * 100)
													: clamp({
															value: parsed,
															min: 0,
															max: 100,
