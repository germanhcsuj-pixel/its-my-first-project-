...
import { TimelineEffectOverlay } from "./timeline-effect-overlay";
...
function ElementInner({
	...
	return (
		...
		>
				...
				<div 
					...
					onDoubleClick={(e) => {
						if (element.type === "text") {
							e.stopPropagation();
							document.getElementById("text-content-editor")?.focus();
						}
					}}
				>
					<ElementContent
			...
			{isSelected && (
				<>
					<div className="border-white pointer-events-none absolute inset-0 z-20 rounded-[0.5rem] border-2 shadow-[0_0_0_1px_rgba(0,0,0,0.5)]" />
					<ResizeHandle
						side="left"
...
function ResizeHandle({
	...
	return (
		<button
			type="button"
			className={`bg-white absolute top-0 bottom-0 z-50 flex w-[0.6rem] items-center justify-center shadow-[0_0_2px_rgba(0,0,0,0.5)] ${isLeft ? "left-0 cursor-w-resize" : "right-0 cursor-e-resize"}`}
			onMouseDown={(e) => handleResizeStart({ e, elementId, side })}
			aria-label={`${isLeft ? "Left" : "Right"} resize handle`}
		>
			<div className="bg-black/50 h-[1.5rem] w-[0.2rem] rounded-full" />
		</button>
	);
}
