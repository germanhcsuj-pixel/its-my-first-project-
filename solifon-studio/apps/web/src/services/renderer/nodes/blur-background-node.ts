...
export class BlurBackgroundNode extends BaseNode<BlurBackgroundNodeParams> {
	...
	async render({
		...
		try {
			offscreen = new OffscreenCanvas(target.width, target.height);
		...
		const originalContext = target.context;
		target.context = offscreenCtx;

		for (const node of this.contentNodes) {
			await node.render({ target, time });
		}

		target.context = originalContext;

		const zoomScale = 1.4;
		const scaledWidth = target.width * zoomScale;
		const scaledHeight = target.height * zoomScale;
		const offsetX = (target.width - scaledWidth) / 2;
		const offsetY = (target.height - scaledHeight) / 2;

		target.context.save();
		target.context.filter = `blur(${this.blurIntensity}px)`;
