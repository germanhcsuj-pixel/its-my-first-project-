...
export type CollectedAudioElement = Omit<
	AudioElement,
	"mediaId" | "id" | "name" | "sourceUrl" | "type"
> & { buffer: AudioBuffer; type: string; sourceType?: string };
			...
			pendingElements.push(
				resolveAudioBufferForElement({
					...
					return {
						...
						muted,
						type: element.type,
						sourceType: (element as any).sourceType,
					};
				}),
			);
		}
...
async function fetchLibraryAudioClip({
	...
	try {
		...
		return {
			...
			isBackground: true, // Library audio is treated as background
		};
	} catch (error) {
		console.warn("Failed to fetch library audio:", error);
		return null;
...
function collectMediaAudioClip({
	...
	return {
		...
		playbackRate: getElementPlaybackRate({ element }),
		elementType: element.type,
		isBackground: element.type === "audio" && (element as any).sourceType === "library",
	};
}
...
function mixAudioChannels({
	...
	const isBackground = element.type === "audio" && (element as any).sourceType === "library";
	const DUCKING_VOLUME = 0.2;
	const RAMP_TIME = 0.3;
	...
	for (let channel = 0; channel < outputChannels; channel++) {
		...
		for (let i = 0; i < resampledLength; i++) {
			...
			if (isBackground && duckingEnvelopes.length > 0) {
				const t = outputIndex / sampleRate;
				let duckMult = 1.0;
				for (const env of duckingEnvelopes) {
					if (t >= env.start && t <= env.end) {
						duckMult = DUCKING_VOLUME;
						break;
					} else if (t >= env.start - RAMP_TIME && t < env.start) {
						const progress = (t - (env.start - RAMP_TIME)) / RAMP_TIME;
						const interpolatedMult = 1.0 - progress * (1.0 - DUCKING_VOLUME);
						duckMult = Math.min(duckMult, interpolatedMult);
					} else if (t > env.end && t <= env.end + RAMP_TIME) {
						const progress = (t - env.end) / RAMP_TIME;
						const interpolatedMult = DUCKING_VOLUME + progress * (1.0 - DUCKING_VOLUME);
						duckMult = Math.min(duckMult, interpolatedMult);
					}
				}
				currentVolume *= duckMult;
			}

			outputData[outputIndex] += interpolated * currentVolume;
		}
	}
}
