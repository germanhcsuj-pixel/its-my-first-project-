...
export class InsertElementCommand extends Command {
	...
	execute(): void {
		...
		if (this.targetTrackId && this.elementId) {
			editor.selection.setSelectedElements({
				elements: [{ trackId: this.targetTrackId, elementId: this.elementId }],
			});
		}
	...
	private validateElementBasics({
		...
		if (requiresMediaId({ element }) && !("mediaId" in element)) {
			...
			toast.error("Failed to add: Element requires mediaId");
			return false;
		}
		...
		if (
			...
			toast.error("Failed to add: Library audio element must have sourceUrl");
			return false;
		}

		if (element.type === "sticker" && !element.iconName) {
			console.error("Sticker element must have iconName");
			toast.error("Failed to add: Sticker element must have iconName");
			return false;
		}

		if (element.type === "text" && !element.content) {
			console.error("Text element must have content");
			toast.error("Failed to add: Text element must have content");
			return false;
		}

		if (typeof element.duration !== "number" || element.duration <= 0) {
			console.error("Element missing valid duration", element.duration);
			toast.error(`Failed to add: Invalid duration (${element.duration})`);
			return false;
		}
	...
	private resolveTracksWithElement({
		...
		if (
			...
			toast.error(`Failed to add: ${element.type} elements cannot be placed on ${trackType} tracks`);
			return null;
		}
	...
	private getTrackTypeForElement({
		...
		if (element.type === "video" || element.type === "image" || element.type === "shape") {
			return "video";
		}
